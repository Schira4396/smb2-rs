...
pub fn respond_challenge_ntlm_v2(server_challenge: [u8; 8], target_info: &[u8], time: i64, creds: &Credentials) -> ChallengeResponse {
    let mut client_challenge: [u8; 8] = [0; 8];
    OsRng.fill(&mut client_challenge);
    // client_challenge = [1, 2, 3, 4, 5, 1, 1, 1];
    // println!("clientchallenge : {:?}", client_challenge);
    ...
    // let ntlm_key:[u8;16] = [161 ,242, 76 ,159, 233, 240, 187, 255, 211, 94, 37, 212 ,126 ,244, 24 ,11];
    // println!("ntlm_key : {:?}", ntlm_key);
    ...
    // println!("nt_proof_string is {:?}",nt_proof_string );
